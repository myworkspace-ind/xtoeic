...
public class HeaderMetaData {
    ...
    /**
     * Pattern Fx or Feedback_x.
     * x: A ~ Z
     * 
     * FA, FB,...: ^F[A-Z]$
     * Feedback_A, Feedback_B, ...: ^Feedback_[A-Z]$
     */
    private static final String FEEDBACK_PATTERN = "^(F|Feedback_)[A-Z]$";

    private String[] headerNames;

    // The first column is No: Sequence no only.
    /** Default column of the Question. */
    private int indexQuestion = 1;

    /** Default column of the Level. */
    private int indexLevel = 2;

    /** Default column of the Score . */
    private int indexScore = 3;

    /** Default column of Not Random checking . */
    private int indexIsNotRandom = 4;

    /** Default column of Question Type . */
    private int indexQuestionType = 5;

    /** Default column of Corrective Answer. */
    private int indexCorrectAnswer = 6;

    /** Default start column of options. */
    private int indexAnswerStart = 7;
    ...
    private void analysisHeader() {
        ...
        for (int i = 0; i < len; i++) {
            ...
            // Check the Feedback
            if ("FA".equalsIgnoreCase(headerNames[i]) || "Feedback_A".equalsIgnoreCase(headerNames[i])) {
                indexFeedbackStart = i;
                
                // Scan next header of Feedback
                i++;
                // while ((i < len) && (headerNames[i].length() == 2)) { // Length of FA, FB,... 2; Or Length of Feedback_A...
                while ((i < len) && (headerNames[i].matches(FEEDBACK_PATTERN))) { // Length of FA, FB,... 2; Or Length of Feedback_A...
                    indexFeedbackEnd = i;
                    i++;
                }
            }
